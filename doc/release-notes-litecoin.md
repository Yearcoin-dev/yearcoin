Yearcoin Core version 0.13.2 is now available from:

  <https://download.yearcoin.org/yearcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/yearcoin-project/yearcoin/issues>
...
updates will be released anymore. Without security updates, using a yearcoin
...
ECDSA signatures inside Yearcoin transactions now use validation using
...
Previous versions of Yearcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Yearcoin Core was to raise the default minimum
relay fee.

Yearcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Yearcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running yearcoind without having to do any manual configuration.
...
Yearcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Yearcoin Core will relay transactions with
...
entirely in Yearcoin Core 0.13, and it is at this time undecided whether the
...
Yearcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Yearcoin Core automatically creates a hidden service to listen on, without
manual configuration. Yearcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Yearcoin Core is listening, and
...
Yearcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Yearcoin
Core to use floating fees. By default, Yearcoin Core will use floating
fees.
...
(default: `0.0002` YRC/kB).

At all times, Yearcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) YRC.
Furthermore, Yearcoin Core will never create transactions paying less than
...
`yearcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /yearcoinrpc>
        ProxyPass http://127.0.0.1:8759/
        ProxyPassReverse http://127.0.0.1:8759/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass yearcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< yearcoinrpc:<password>
    </Location>
...
Restarting yearcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in yearcoin.conf
...
yearcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/yearcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Yearcoin Core code base has
...
- `yearcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `yearcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Yearcoin scripting system that in combination with
...
Yearcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Yearcoin Core.
...
The two phases are distinct in the Yearcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Yearcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Yearcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Yearcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of yearcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of yearcoins being spent is exactly
...
- **Increased security for multisig:** Yearcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Yearcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Yearcoin's current ledger (called the UTXO set),
  ...
  note that Yearcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Yearcoin users to individually opt-in to almost any change in the Yearcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Yearcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Yearcoin
client to Yearcoin Core 0.13.2 to signal for segwit support. When a
...
Since Yearcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `yearcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `yearcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
